...
class Solution:
    ...
    def maxCoins(self, nums: List[int]) -> int:
        ...
        # calculate all on diagonal from top-left to down-right
        # then the next diagonal line, etc, till you reach the vey end
        # let d be the "distance" from the center diagonal
        for d in range(0, n):
            for idx in range(0, n - d):
                # we calculate dp[i][j] here
                i, j = idx, idx + d
                score = 0
                for k in range(i, j + 1):
                    cur_score = (
                            dp[i][k-1] +  # result for the left
                            dp[k+1][j] +  # result for the right
                            # result from popping the k-th
                            (nums[k] *
                             (nums[i - 1] if i - 1 >= 0 else 1) *
